...
function EditorSocketIOServer (document, operations, docId, mayWrite) {
   ...
   */
  Server.call(this, document, operations); // 根据document和operations初始化服务
  this.users = {};
  this.docId = docId;
...
EditorSocketIOServer.prototype.addClient = function (socket) {
  var self = this;
  socket
    .join(this.docId) // 用户加到一个docId房间
    .emit('doc', { // 给新加入用户发送初始化数据
      str: this.document,
      revision: this.operations.length, // 一个操作版本号取operations长度
      clients: this.users
    })
